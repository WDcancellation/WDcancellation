...
export default function App() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      if (resRequests.ok) setRequests(dataRequests);

      // Dropdowns
      const resDropdowns = await fetch('/api/dropdowns', { headers });
      const dataDropdowns = await resDropdowns.json();
      if (resDropdowns.ok) {
      ...
      if (resLabels.ok) setLabelNames(dataLabels);

      // Committees
      const resCommittees = await fetch('/api/committees', { headers });
      const dataCommittees = await resCommittees.json();
      if (resCommittees.ok) setCommittees(dataCommittees);

      // Custom Fields
      const resCF = await fetch('/api/custom-fields', { headers });
      const dataCF = await resCF.json();
      if (resCF.ok && Array.isArray(dataCF)) setCustomFields(dataCF);

      // Audit logs (Admin only)
      if (currentUser?.role === 'admin') {
        const resLogs = await fetch('/api/logs/audit', { headers });
        const dataLogs = await resLogs.json();
        if (resLogs.ok) setAuditLogs(dataLogs);
      }
    } catch (err) {
